...
from PyQt6.QtWidgets import (
    QDialog, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QSpinBox, QCheckBox, QPushButton
)
...
class SaveDialog(QDialog):
    ...
    def __init__(self, parent=None, *,
        ...
        self.sb_w = QSpinBox(); self.sb_w.setRange(1, 8192)
        self.sb_h = QSpinBox(); self.sb_h.setRange(1, 8192)
        if target_width: self.sb_w.setValue(int(target_width))
        if target_height: self.sb_h.setValue(int(target_height))
        for w in (self.sb_w, self.sb_h):
            w.setEnabled(self._size_enabled)
        row_sz.addWidget(QLabel("W")); row_sz.addWidget(self.sb_w)
        row_sz.addWidget(QLabel("H")); row_sz.addWidget(self.sb_h)
        v.addLayout(row_sz)

        # Buttons
        row_btn = QHBoxLayout(); row_btn.addStretch(1)
        ok = QPushButton("確定"); cancel = QPushButton("取消")
        ok.clicked.connect(self.accept); cancel.clicked.connect(self.reject)
        row_btn.addWidget(ok); row_btn.addWidget(cancel)
        v.addLayout(row_btn)

    def result_payload(self) -> dict:
        name = self.ed_name.text().strip()
        q = int(self.sb_quality.value())
        auto = bool(self.cb_auto_fps.isChecked()) if hasattr(self, 'cb_auto_fps') else False
